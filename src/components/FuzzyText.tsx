  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const computedFontFamily =
        fontFamily === 'inherit'
          ? window.getComputedStyle(canvas).fontFamily || '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif'
          : fontFamily;
      ...
      if (typeof fontSize === 'number') {
        ...
        numericFontSize = parseFloat(computedSize) || 36;
      ...
      const extraWidthBuffer = 16;
      const offscreenWidth = Math.max(1, textBoundingWidth + extraWidthBuffer);
      const offscreenHeight = Math.max(1, tightHeight);
      ...
      const startGlitchLoop = () => {
        ...
        }, glitchInterval + Math.random() * 500);
      };
      ...
      const run = (timestamp: number) => {
        ...
        if (forceIntensity !== null && forceIntensity !== undefined) {
          targetIntensity = forceIntensity;
        } else if (isClicking) {
          targetIntensity = 1;
        } else if (isGlitching) {
        ...
        if (direction === 'horizontal') {
          ...
          try {
            ...
            for (let i = 0; i < offscreenWidth + fuzzRange; i++) {
              ...
              ctx.clearRect(i, -verticalMargin, 1, tightHeight + 2 * verticalMargin);
