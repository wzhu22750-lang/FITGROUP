      ...
      <div className="bg-neon p-8 border-4 border-ink shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center">
        <div className="border-4 border-ink p-1 bg-white mb-4">
          {user?.user_metadata?.photoURL ? (
            <img src={user.user_metadata.photoURL} className="w-24 h-24 object-cover" />
          ) : (
        ...
        <h2 className="text-3xl font-black text-ink tracking-tighter uppercase italic leading-none">{user?.user_metadata?.displayName || 'User'}</h2>
